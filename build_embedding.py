...
def get_custom_data(base_path, device):
    ...
    for utt_folder in os.listdir(base_path):
        ...
        data['linear_spec'] = lin_spec

        batch.append(
            TTSDataItem(**data)
        )

    #batch = th.concat([it.mel_spec for it in batch], device = device)
...
batch, T_actual  = get_custom_data('processed/custom_4096_512', device = device)
model = th.load('output/savepoints/speaker_encoder_mode_1400.pth', map_location=device).to(device)
model.device = device
model.eval()
...
with th.no_grad():
    ...
    th.save(output, os.path.join(EMBEDDING_OUT_DIR, 'Demetrius.pth'))
